...
class Printer {
     ...
     */
    public function cut($mode = Printer::CUT_FULL, $lines = 3, $serie = null) {
        // TODO validation on cut() inputs
        if ($serie == "LR2000") {
            $this->connector->write(self::ESC . "\x69" . chr($lines));
        } else {
            $this->connector->write(self::GS . chr($mode) . chr($lines));
        }
    }
